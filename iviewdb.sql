...
CREATE TABLE IF NOT EXISTS `iviewdb`.`respondent` (
  ...
  PRIMARY KEY (`id`),
  INDEX `respondentScriptId_idx` (`script_id` ASC),
  CONSTRAINT `respondentScriptId`
    FOREIGN KEY (`script_id`)
    REFERENCES `iviewdb`.`scripts` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
...
-- -----------------------------------------------------
-- Data for table `iviewdb`.`users`
-- -----------------------------------------------------
START TRANSACTION;
USE `iviewdb`;
INSERT INTO `iviewdb`.`users` (`id`, `username`, `displayname`, `password`, `email`, `lastSaved`) VALUES (1, 'admin', 'Administrator', '47db0f64e7eb6b5c856ad14a38c3d01ec857c3275901e3b2918847f918333dee', NULL, NULL);
